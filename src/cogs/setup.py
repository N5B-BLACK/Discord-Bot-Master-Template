...
Split into 5 pages (Discord allows a max of 5 rows per message):
- Page 1: mod role, warn log channel, welcome channel, auto role
- Page 2: AI channel, support role (tickets), ticket log channel
- Page 3: voice logs (join/leave, switch, disconnect, mute)
- Page 4: ban/unban, server join/leave, message deletion, timeout
- Page 5: kick, setup update log
"""
...
def _page1_embed(settings: dict) -> discord.Embed:
    description = (
        ...
        f"3️⃣ Welcome channel: {_mention(settings, 'welcome_channel_id', 'channel')}\n"
        f"4️⃣ Auto-role: {_mention(settings, 'auto_role_id', 'role')}\n\n"
        "Click **Next ▶️** for more settings."
    )
    return discord.Embed(title="⚙️ Bot Settings (1/5)", description=description, color=discord.Color.blurple())


def _page2_embed(settings: dict) -> discord.Embed:
    description = (
        "AI and ticket settings.\n\n"
        f"5️⃣ AI channel (/ask): {_mention(settings, 'ai_chat_channel_id', 'channel')}\n"
        f"6️⃣ Support role (tickets): {_mention(settings, 'ticket_support_role_id', 'role')}\n"
        f"7️⃣ Ticket log channel: {_mention(settings, 'ticket_log_channel_id', 'channel')}"
    )
    return discord.Embed(title="⚙️ Bot Settings (2/5)", description=description, color=discord.Color.blurple())
...
def _page3_embed(settings: dict) -> discord.Embed:
    description = (
        ...
        f"8️⃣ Voice join/leave: {_mention(settings, 'voice_join_leave_log_channel_id', 'channel')}\n"
        f"9️⃣ Voice switch: {_mention(settings, 'voice_switch_log_channel_id', 'channel')}\n"
        f"🔟 Voice disconnect: {_mention(settings, 'voice_disconnect_log_channel_id', 'channel')}\n"
        f"1️⃣1️⃣ Voice mute/unmute: {_mention(settings, 'voice_mute_log_channel_id', 'channel')}"
    )
    return discord.Embed(title="⚙️ Bot Settings (3/5)", description=description, color=discord.Color.blurple())
...
def _page4_embed(settings: dict) -> discord.Embed:
    description = (
        ...
        f"1️⃣2️⃣ Ban/unban: {_mention(settings, 'ban_unban_log_channel_id', 'channel')}\n"
        f"1️⃣3️⃣ Server join/leave: {_mention(settings, 'server_join_leave_log_channel_id', 'channel')}\n"
        f"1️⃣4️⃣ Message deletion: {_mention(settings, 'msg_deleted_log_channel_id', 'channel')}\n"
        f"1️⃣5️⃣ Timeout: {_mention(settings, 'timeout_log_channel_id', 'channel')}"
    )
    return discord.Embed(title="⚙️ Bot Settings (4/5)", description=description, color=discord.Color.blurple())


def _page5_embed(settings: dict) -> discord.Embed:
    description = (
        "Kick log, voice deafen log, and a log that records any change made to these settings themselves.\n\n"
        f"1️⃣6️⃣ Kick log: {_mention(settings, 'kicked_log_channel_id', 'channel')}\n"
        f"1️⃣7️⃣ Settings update log: {_mention(settings, 'setup_update_log_channel_id', 'channel')}\n"
        f"1️⃣8️⃣ Voice deafen/undeafen log: {_mention(settings, 'voice_deafen_log_channel_id', 'channel')}"
    )
    return discord.Embed(title="⚙️ Bot Settings (5/5)", description=description, color=discord.Color.blurple())
...
class SetupViewPage1(discord.ui.View):
    ...
    @discord.ui.select(
        cls=discord.ui.ChannelSelect,
        placeholder="3️⃣ Select the welcome channel",
        channel_types=[discord.ChannelType.text],
        row=2,
    )
    ...
    @discord.ui.select(cls=discord.ui.RoleSelect, placeholder="4️⃣ Select the auto-role for new members", row=3)
    async def auto_role_select(self, interaction: discord.Interaction, select: discord.ui.RoleSelect):
        role = select.values[0]
        await _apply_setting(interaction, "auto_role_id", role.id, "Auto-role", role.mention)
...
class SetupViewPage2(discord.ui.View):
    ...
    @discord.ui.select(
        cls=discord.ui.ChannelSelect,
        placeholder="5️⃣ Select the /ask (AI) channel",
        channel_types=[discord.ChannelType.text],
        row=0,
    )
    ...
    @discord.ui.select(cls=discord.ui.RoleSelect, placeholder="6️⃣ Select the support role (tickets)", row=1)
    ...
    @discord.ui.select(
        cls=discord.ui.ChannelSelect,
        placeholder="7️⃣ Select the ticket log channel (closed ticket transcripts)",
        channel_types=[discord.ChannelType.text],
        row=2,
    )
...
class SetupViewPage3(discord.ui.View):
    ...
    @discord.ui.select(
        cls=discord.ui.ChannelSelect,
        placeholder="8️⃣ Voice join/leave log channel",
        channel_types=[discord.ChannelType.text],
        row=0,
    )
    ...
    @discord.ui.select(
        cls=discord.ui.ChannelSelect,
        placeholder="9️⃣ Voice switch log channel",
        channel_types=[discord.ChannelType.text],
        row=1,
    )
    ...
    @discord.ui.select(
        cls=discord.ui.ChannelSelect,
        placeholder="🔟 Voice disconnect log channel",
        channel_types=[discord.ChannelType.text],
        row=2,
    )
    ...
    @discord.ui.select(
        cls=discord.ui.ChannelSelect,
        placeholder="1️⃣1️⃣ Voice mute/unmute log channel",
        channel_types=[discord.ChannelType.text],
        row=3,
    )
...
class SetupViewPage4(discord.ui.View):
    ...
    @discord.ui.select(
        cls=discord.ui.ChannelSelect,
        placeholder="1️⃣2️⃣ Ban/unban log channel",
        channel_types=[discord.ChannelType.text],
        row=0,
    )
    ...
    @discord.ui.select(
        cls=discord.ui.ChannelSelect,
        placeholder="1️⃣3️⃣ Server join/leave log channel",
        channel_types=[discord.ChannelType.text],
        row=1,
    )
    ...
    @discord.ui.select(
        cls=discord.ui.ChannelSelect,
        placeholder="1️⃣4️⃣ Message deletion log channel",
        channel_types=[discord.ChannelType.text],
        row=2,
    )
    ...
    @discord.ui.select(
        cls=discord.ui.ChannelSelect,
        placeholder="1️⃣5️⃣ Timeout log channel",
        channel_types=[discord.ChannelType.text],
        row=3,
    )
...
class SetupViewPage5(discord.ui.View):
    ...
    @discord.ui.select(
        cls=discord.ui.ChannelSelect,
        placeholder="1️⃣6️⃣ Kick log channel",
        channel_types=[discord.ChannelType.text],
        row=0,
    )
    ...
    @discord.ui.select(
        cls=discord.ui.ChannelSelect,
        placeholder="1️⃣7️⃣ Settings update log channel",
        channel_types=[discord.ChannelType.text],
        row=1,
    )
    ...
    @discord.ui.select(
        cls=discord.ui.ChannelSelect,
        placeholder="1️⃣8️⃣ Voice deafen/undeafen log channel",
        channel_types=[discord.ChannelType.text],
        row=2,
    )
